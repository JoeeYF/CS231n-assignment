...
def rnn_step_backward(dnext_h, cache):
    ...
    dhiden = (1-next_h*next_h)*dnext_h #(N, H)
    dx = np.dot(dhiden,Wx.T)#(N, D)
    dWx = np.dot(x.T,dhiden)  # (D, H)
    dprev_h = np.dot(dhiden,Wh.T)
    dWh = np.dot(prev_h.T,dhiden)
    db = np.sum(dhiden,axis=0)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
        ...
        dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dnext_h, cache[i])
        dx[:,i,:]=dx_
        dWx+=dWx_
        dWh+=dWh_
        db+=db_
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    N,H=prev_h.shape
    next_ = np.dot(prev_h,Wh)+np.dot(x,Wx)+b #N,4H
    i = sigmoid(next_[:,:H])
    f = sigmoid(next_[:,H:2*H])
    o = sigmoid(next_[:,2*H:3*H])
    g = np.tanh(next_[:,3*H:4*H])
    next_c = f*prev_c+i*g
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_ = np.hstack([i*(1-i)*di, f*(1-f)*df, o*(1-o)*do, (1-g*g)*dg]) #N,4H
    
    dWx = np.dot(x.T,dnext_)  #D,4H
    dWh = np.dot(prev_h.T,dnext_) #H, 4H
    dx = np.dot(dnext_,Wx.T)  #N, D
    dprev_h= np.dot(dnext_,Wh.T)
    db =  np.sum(dnext_, axis=0)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
        dnext_h=dprev_h+dh[:,i,:]
        dx_, dprev_h, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dnext_h, dprev_c, cache[i])
#         dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dnext_h, cache[i])
        dx[:,i,:]=dx_
        dWx+=dWx_
        dWh+=dWh_
